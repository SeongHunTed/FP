...
int readRecord(FILE *fp, STUDENT *s, int rrn)
{
	...
	memset(recordbuf, 0, 80);
	fseek(fp, HEADER_SIZE + RECORD_SIZE * rrn, SEEK_SET);
	fread(recordbuf, 80, 1, fp);
	printf("%s", recordbuf);
	unpack(recordbuf, s);
	// if(fread(recordbuf, 80, 1, fp) > 0){
	// 	printf("fuck");
	// 	for(int i = 0; i < 80; i++){
...
{	
	...
	if(strcmp(enumcheck, "NAME") == 0) return NAME;
	if(strcmp(enumcheck, "DEPT") == 0) return DEPT;
	if(strcmp(enumcheck, "ADDR") == 0) return ADDR;
	if(strcmp(enumcheck, "EMAIL") == 0) return EMAIL;
}